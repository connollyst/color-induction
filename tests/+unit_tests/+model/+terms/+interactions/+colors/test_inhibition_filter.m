...
function test_opponent_color_inhibition_filter_with_2_color_channels
    ...
    % Then
    expected_weight = 0.2;
    assertColorFilterSize(actual_filter)
    assertColorFilterValues(expected_weight, actual_filter)
end
...
function assertColorFilterSize(actual_filter)
    actual_size   = size(actual_filter);
    expected_size = [1 1 3];
    assertEqual(expected_size, actual_size);
end

function assertColorFilterValues(expected_weight, actual_filter)
