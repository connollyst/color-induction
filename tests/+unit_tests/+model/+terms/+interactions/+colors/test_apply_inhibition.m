...
% themselves, but that there is no inhibition between the pairs.
    ...
    I_expected(:,:,[1,2]) = model.data.convolutions.optimal( ...
                    I_in(:,:,[1,2]), interactions.color.inhibition_filter ...
                 );
    I_expected(:,:,[3,4]) = model.data.convolutions.optimal( ...
                    I_in(:,:,[3,4]), interactions.color.inhibition_filter ...
                 );
    assertEqual(I_out, I_expected);
end
