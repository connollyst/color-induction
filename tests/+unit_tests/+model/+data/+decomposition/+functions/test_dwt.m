...
function test_dwt_decomposition_dimensions_default
    ...
    assertEqual(size(decompositions, 3), size(I, 3));
...
function test_dwt_signal_for_synthetic_black_and_white_image
    % Given
    n_scales = 1;
    I = make_synthetic_I();
    I = model.data.color.rgb2itti(I);
...
function test_dwt_single_and_double_opponent_dimensions_at_2_scales
    ...
    assertEqual(size(signal, 3), size(I, 3));
