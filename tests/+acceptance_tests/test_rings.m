...
function test_neon_rings_dwt
    % Given
    G = imresize(imread('tests/data/input/test-circle-neon-a.png'), 0.2); % Perceived greener
    B = imresize(imread('tests/data/input/test-circle-neon-b.png'), 0.2); % Perceived bluer
    ...
    % Then
    mask = im2double(imresize(imread('tests/data/input/test-circle-neon-mask.png'), 0.2));
    G_masked = G_out .* mask;
    B_masked = B_out .* mask;
    lims_max = max(max(G_masked(:)), max(B_masked(:)));
    lims_min = min(min(G_masked(:)), min(B_masked(:)));
    lims     = [lims_min lims_max];
    do_plot(2, 'Test Circle A L-D', lims, G_masked(:,:,1))
    do_plot(3, 'Test Circle A R-G', lims, G_masked(:,:,2))
    do_plot(4, 'Test Circle A B-Y', lims, G_masked(:,:,3))
