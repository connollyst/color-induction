function [data,normal_max,normal_min] = curv_normalization(data, struct)
...
switch type
	case ('all')
		...
		for i=1:ncells
			normal_max_v(i)=max(data{i}(:),[],1);
			normal_min_v(i)=min(data{i}(:),[],1);
		end
		...
		if normal_max==normal_min
			...
			for i=1:ncells
				data{i}=((data{i}-normal_min)/(normal_max-normal_min))*(factor_normal-shift)+shift;
			end
		end
			% -------------------------------
	case ('scale')
		...
		for s=1:n_scales
			if normal_max(s)==normal_min(s)
				for i=1:ncells
					data{i}(:,:,s,:)=1.02; % El minim segons Li1998
				end
			else
				for i=1:ncells
					data{i}(:,:,s,:)=((data{i}(:,:,s,:)-normal_min(s))/(normal_max(s)-normal_min(s)))*(factor_normal-shift)+shift;
				end
			end
		end
			% -------------------------------
...
for i=1:ncells
	data{i}(data{i}==shift)=0; % Per posar a zero el que era zero inicialment (Li1998)
end
