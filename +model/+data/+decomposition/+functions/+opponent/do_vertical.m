...
%                 LDRGBY_v(column, row, color, scale)
    ...
    for scale=1:config.wave.n_scales
        LDRGBY_t2b = model.data.decomposition.functions.opponent.rf.utils.filter( ...
                        rgb,                                    ...
                        top2bottom_filter_ON(scale, config),    ...
                        top2bottom_filter_OFF(scale, config)    ...
                     );
        LDRGBY_b2t = model.data.decomposition.functions.opponent.rf.utils.filter( ...
                        rgb,                                    ...
...
function f = top2bottom_filter_ON(scale, config)
    f =   model.data.decomposition.functions.opponent.rf.oriented.center_top_middle(scale, config) ...
...
function f = bottom2top_filter_ON(scale, config)
    f =   model.data.decomposition.functions.opponent.rf.oriented.center_bottom_middle(scale, config) ...
...
function f = top2bottom_filter_OFF(scale, config)
    f =   model.data.decomposition.functions.opponent.rf.oriented.surround_top_middle(scale, config) ...
...
function f = bottom2top_filter_OFF(scale, config)
    f =   model.data.decomposition.functions.opponent.rf.oriented.surround_bottom_middle(scale, config) ...
