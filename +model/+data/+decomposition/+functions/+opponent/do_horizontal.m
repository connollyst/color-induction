...
%                 LDRGBY_h(column, row, color, scale)
    ...
    for scale=1:config.wave.n_scales
        LDRGBY_l2r = model.data.decomposition.functions.opponent.rf.utils.filter(rgb, ...
                        left2right_filter_ON(scale, config), ...
                        left2right_filter_OFF(scale, config) ...
                     );
        LDRGBY_r2l = model.data.decomposition.functions.opponent.rf.utils.filter(rgb, ...
...
function f = left2right_filter_ON(scale, config)
    f =   model.data.decomposition.functions.opponent.rf.oriented.center_middle_left(scale, config) ...
...
function f = right2left_filter_ON(scale, config)
    f =   model.data.decomposition.functions.opponent.rf.oriented.center_middle_right(scale, config) ...
...
function f = left2right_filter_OFF(scale, config)
    f =   model.data.decomposition.functions.opponent.rf.oriented.surround_middle_left(scale, config) ...
...
function f = right2left_filter_OFF(scale, config)
    f =   model.data.decomposition.functions.opponent.rf.oriented.surround_middle_right(scale, config) ...
